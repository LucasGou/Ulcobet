...
<head>
	...
	<title>Roulette des gains</title>
</head>
<body>
	<div id='header'>
...
foreach($select10 as $s10){
	echo "<h3 id='Jeton'>Il vous reste ".$s10['JetonGain']." Jetons</h3>";
}



$sth = $base->prepare('select count(*) from Gain');
$sth->execute(array());
$select = $sth->fetchAll();
...
for($i = 1; $i <= 6; $i++){
	...
	foreach($select2 as $s2){
		if($i==1){
			$_SESSION['rouge'] = $s2['IdGain'];
		}
		if($i==2){
			$_SESSION['orange'] = $s2['IdGain'];
		}
		if($i==3){
			$_SESSION['jaune'] = $s2['IdGain'];
		}
		if($i==4){
			$_SESSION['bleuf'] = $s2['IdGain'];
		}
		if($i==5){
			$_SESSION['bleu'] = $s2['IdGain'];
		}
		if($i==6){
			$_SESSION['bleuc'] = $s2['IdGain'];
		}
	...
	?>
        ...
        <div id="wheel">
            ...
            <?php
					...
					$select2 = $sth2->fetchAll();
					foreach($select2 as $s2){
					echo "<div class='sec' title='".$s2['LibelleGain']."'><span class='fa fa-bell-o' >";
					echo $s2['LibelleGain'];
											}
                 ...
                 echo "</span></div>";
					...
					$select2 = $sth2->fetchAll();
					foreach($select2 as $s2){
					echo "<div class='sec' title='".$s2['LibelleGain']."'><span class='fa fa-comment-o'>";
					echo $s2['LibelleGain'];
											}
                 ...
                 echo "</span></div>";
					...
					$select2 = $sth2->fetchAll();
					foreach($select2 as $s2){
					 echo "<div class='sec' title='".$s2['LibelleGain']."'><span class='fa fa-smile-o' >";
					echo $s2['LibelleGain'];
											}
                 ...
                 echo "</span></div>";
					...
					$select2 = $sth2->fetchAll();
					foreach($select2 as $s2){
					echo "<div class='sec' title='".$s2['LibelleGain']."'><span class='fa fa-heart-o' >";
					echo $s2['LibelleGain'];
											}
                 ...
                 echo "</span></div>";
					...
					$select2 = $sth2->fetchAll();
					foreach($select2 as $s2){
					echo "<div class='sec' title='".$s2['LibelleGain']."'><span class='fa fa-star-o'>";
					echo $s2['LibelleGain'];
											}
                 ...
                 echo "</span></div>";
					...
					foreach($select2 as $s2){
					echo "<div class='sec' title='".$s2['LibelleGain']."'><span class='fa fa-lightbulb-o' >";
					echo $s2['LibelleGain'];
											}
